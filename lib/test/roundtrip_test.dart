...
ZegelResult _roundtrip(
  ...
  // Step 2: extract
  final extractResult = ZegelReader.extract(fileBytes, key);
...
void main() {
  group('Roundtrip tests', () {
    ...
    group('content types', () {
      ...
      test('JSON content', () {
        final jsonObj = {'key': 'value', 'number': 42, 'array': [1, 2, 3]};
    ...
    group('with compression', () {
      ...
      test('compressed file is smaller than uncompressed for compressible data', () {
        ...
        final compressed =
            ZegelWriter.seal(content, masterKey, options: optionsCompressed);
        final uncompressed =
            ZegelWriter.seal(content, masterKey, options: optionsUncompressed);
    ...
    group('determinism', () {
      test('same inputs produce same output with fixed salt', () {
        ...
        final file1 = ZegelWriter.seal(content, masterKey, options: options);
        final file2 = ZegelWriter.seal(content, masterKey, options: options);
