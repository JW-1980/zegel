...
void main() {
  group('SealPreflight', () {
    test('empty file produces an error warning', () {
      ...
      expect(result.warnings.any((w) => w.code == 'empty_file'), isTrue);
    ...
    test('oversize file emits an info warning', () {
      ...
      expect(result.warnings.any((w) => w.code == 'oversize_file'), isTrue);
    });
    ...
    test('PII-bearing text triggers a warning', () {
      ...
      expect(result.warnings.any((w) => w.code == 'pii_detected'), isTrue);
    ...
    test('weak password triggers a warning', () {
      ...
      expect(result.warnings.any((w) => w.code == 'weak_password'), isTrue);
    });
    ...
    test('strong password and clean text produce no warnings', () {
      ...
      expect(result.warnings.any((w) => w.code == 'weak_password'), isFalse);
    });
  });
}
