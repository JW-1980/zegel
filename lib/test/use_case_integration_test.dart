...
List<Uint8List> _getLeafHashes(Uint8List fileBytes) {
  ...
  for (int i = 0; i < blockCount; i++) {
    ...
    hashes.add(
      Uint8List.fromList(fileBytes.sublist(entryOffset + 1, entryOffset + 33)),
    );
  }
  return hashes;
}
...
void main() {
  ...
  group('Use Case: House Purchase Contract', () {
    ...
    test(
      'full contract workflow: seal, attest by multiple parties, verify policy',
      () {
        // 1. Seal the contract
        ...
        expect(
          policyResult.allRolesFulfilled,
          isTrue,
  ...
  group('Use Case: Financial Audit', () {
    ...
    test('batch verify quarterly statements', () {
      ...
      for (final quarter in quarters) {
        final content = Uint8List.fromList(
          utf8.encode(
            'Financial statement for $quarter 2025\n'
        ...
        sealedStatements.add(MapEntry('statement_${quarter}_2025', fileBytes));
      }

      // Batch verify all
      final results = BatchOperations.batchVerify(sealedStatements, masterKey);
    ...
    test('manifest for audit package', () {
      ...
      for (final quarter in ['Q1', 'Q2', 'Q3', 'Q4']) {
        final content = Uint8List.fromList(utf8.encode('Statement $quarter'));
        final options = ZegelOptions(
          contentType: 'text/plain',
  ...
  group('Use Case: Academic Credentials', () {
    ...
    test('issue and verify diploma', () {
      ...
      expect(
        inspection.publicMetadata!['issuer'],
  ...
  group('Use Case: Government Classified', () {
    ...
    test('classify, distribute, declassify', () {
      ...
      expect(classInfo['level'], equals(ZegelFormat.classificationTopSecret));
  ...
  group('Use Case: Journalism Excerpt', () {
    ...
    test('prove quote exists in sealed source', () {
      ...
      expect(
        proof['merkle_root'],
        equals(
          merkleRoot.map((b) => b.toRadixString(16).padLeft(2, '0')).join(),
        ),
      );
    });
  });
  ...
  group('Use Case: Insurance Claim', () {
    ...
    test('seal damage photos with GPS metadata', () {
      ...
      final fileBytes = ZegelWriter(masterKey, options).seal(photoContent);
  ...
  group('Use Case: Whistleblower', () {
    ...
    test('seal with split key for secure submission', () {
      ...
      final verifyResult = const ZegelReader().verify(fileBytes, reconstructed);
      expect(
        verifyResult.valid,
        isTrue,
  ...
  group('Use Case: Supply Chain', () {
    ...
    test('track provenance through supply chain', () {
      ...
      final sealedProduct = ZegelWriter(
        masterKey,
        ZegelOptions(
          contentType: 'application/octet-stream',
  ...
  group('Use Case: Contract Negotiation', () {
    ...
    test('multiple versions with attestations at each stage', () {
      ...
      final draft1Bytes = ZegelWriter(masterKey, draft1Options).seal(draft1);
      ...
      final draft2Bytes = ZegelWriter(masterKey, draft2Options).seal(draft2);
  ...
  group('Use Case: Digital Art Provenance', () {
    ...
    test('seal artwork with creator attestation and provenance', () {
      ...
      final fileBytes = ZegelWriter(masterKey, options).seal(artContent);
      ...
      expect(attestation['role'], equals(ZegelFormat.roleOwner));
      expect(
        Attestation.verifyRoleAttestation(attestation, merkleRoot, artistKey),
        isTrue,
      );
  ...
  group('Use Case: Medical Records', () {
    ...
    test('seal patient record with selective disclosure for specialist', () {
      ...
      final cardioResult = const ZegelReader().extractWithToken(
        fileBytes,
        cardioToken,
      );
  ...
  group('Use Case: Legal Discovery', () {
    ...
    test('seal evidence, redact privileged content, verify chain', () {
      ...
      final fileBytes = ZegelWriter(masterKey, options).seal(evidence);
      ...
      final redacted = Redaction.redactBlocks(fileBytes, masterKey, [1, 2]);
  ...
  group('Use Case: IP / Patents', () {
    ...
    test('timestamp invention disclosure for prior art', () {
      ...
      final fileBytes = ZegelWriter(masterKey, options).seal(disclosure);
      ...
      expect(tsValid, isTrue, reason: 'Trusted timestamp should verify');
